...
public class TokenizerProperty {
  ...
  /**
   * Checks wether a given flag is in the current flag mask. That means that the
  ...
  /**
   * Constructs a <code>TokenProperty</code> object with a set of type, image(s), 
   * companion object and flags with an associated flag mask ({@see #setFlags(int, int)} 
   * for details). 
   *
   * @param type        the property type
   * @param images      the characterising images of a lexical element
